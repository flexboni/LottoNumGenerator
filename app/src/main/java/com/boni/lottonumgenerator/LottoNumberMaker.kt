...
object LottoNumberMaker {
     ...
     */
    fun getRandomLottoNumberd(): MutableList<Int> {
        ...
        for (i in 1..6) {
            ...
            do {
                ...
                // lottoNumber 에 Number 변수의 값이 없을때 까지 반복
    ...
    private fun getRandomLottoNumber(): Int {
        // Random.nextInt 는 0 부터 전달받은 파라미터 값 미만의 번호를 생성
        // ex) Random().nextInt(10) 은 0~9까지의 무작위 수를 변환
        // 1~45까지의 번호를 생성하려면 파라미터에 45를 넣고 결과값의 1을 더한다.
    ...
    fun getShuffleLottoNumbers(): MutableList<Int> {
        ...
        // 1~45 까지 for 문 돌면서 리스트에 로또 번호 저장
        for (number in 1..45) {
            list.add(number)
        }
    ...
    fun getLottoNumbersFromHash(str: String): MutableList<Int> {
        ...
        // SimpleDateFormat 은 날짜의 시간값을 포맷호된 텍스트 형태로 바꿔주는 클래스
